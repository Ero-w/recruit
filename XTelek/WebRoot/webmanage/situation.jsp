...
$(document).ready(function(){
		...
		$("#list_data").datagrid({
			...
			loadMsg:'数据加载中,请等待...',
			url:"../proxy.action?targetAction=getDeleiveryListAction",
			method:"post",
			...
			if (rows!=null && rows.length>0) { 
				...
				$.messager.confirm("系统提示","您是否确认初审通过?",function(yes_no){
                    ...
                    success : function(result) {//返回数据根据结果进行相应的处理  
                        if (result=="11") {
                        	$.messager.show({title:"执行结果",msg:"已通知这"+rows.length+"位同学通过简历筛选.",showType:"show"});
                        	$("#list_data").datagrid("load",form2Json("form_search"));
                        } else {  
	...
	<table id="list_data" class="easyui-datagrid"
		...
		<thead>
			<tr>
				...
				<th data-options="field:'filepath',width:'80px',align:'center',title:'简历下载',formatter:download,width:'100px'"></th>
							...
							<tbody>
								<tr>
									<td style="width:108px">
										<a id="toolber_update" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-reload',plain:true">通过审核</a>
									</td>
								</tr>
							</tbody>
						</table>
