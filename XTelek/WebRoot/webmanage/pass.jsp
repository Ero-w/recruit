...
$(document).ready(function(){
		...
		$("#list_data").datagrid({
			...
			loadMsg:'数据加载中,请等待...',
			url:"../proxy.action?targetAction=getDeliveryAction",
			method:"post",
		...
		$("#toolber_info").click(function(){
			...
			if (rows!=null && rows.length>0) { 
				...
				/**
				$.messager.confirm("系统提示","您是否确认初审通过?",function(yes_no){
					if (yes_no) { 
					$.ajax({  
                    type :"POST",  //提交方式  
                    url : "../proxy.action?targetAction=updateDeliveryStateAction",  
		...
		$("#toolber_update").click(function(){
			...
			if (rows!=null && rows.length>0) { 
				...
				$.messager.confirm("系统提示","请仔细确认是否录取?",function(yes_no){
					if (yes_no) { 
					$.ajax({  
                    type :"POST",  //提交方式  
                    url : "../proxy.action?targetAction=updateDeliveryState2Action",  
                    data : {list:list},//数据，这里使用的是Json格式进行传输  
                    success : function(result) {//返回数据根据结果进行相应的处理  
                        if (result=="11") {
                        	$.messager.show({title:"执行结果",msg:"已通知这"+rows.length+"位同学成功被录取.",showType:"show"});
                        	$("#list_data").datagrid("load",form2Json("form_search"));
                        } else {  
                           $.messager.show({title:"执行失败",msg:"请尝试重新删除或者刷新页面",showType:"show"});
                        }  
	...
	function download(value,rowdata,rowindex) { 
    	return "<a target='_blank' href='../proxy.action?targetAction=downFileAction&email="+rowdata.email+"'>下载</a>";
    }
	</script>
</head>
	...
	<table id="list_data" class="easyui-datagrid"
		...
		<thead>
			<tr>
				...
				<th data-options="field:'filepath',width:'80px',align:'center',width:'100px',title:'简历下载',formatter:download"></th>
					...
					<td style="text-align:left;">
						<table cellpadding="0" cellspacing="0">
							<tbody>
								<tr>
									<td style="width:80px">
										<a id="toolber_info" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-test',plain:true">笔试通知</a>
									</td>
									...
									<td style="width:8px"><div class="datagrid-btn-separator"></div></td>
									<td style="width:80px">
										<a id="toolber_update" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-reload',plain:true">录取</a>
									</td>
								</tr>
							</tbody>
						</table>
