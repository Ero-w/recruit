...
public class UploadAction  extends HttpServlet {
	...
	private static final long serialVersionUID = 7625344750098604641L;
		...
		try {
			...
			for(int i=0;i<list.size();i++){
				...
				if (item.isFormField()) { //获取表单元素是否为文件上传标签
					...
					fileRealPath="D:\\"+"\\"+"file\\"+"\\"+fileName;//文件上传路径
					fileHttpPath="upload/"+fileName;//文件下载路径
			...
			int a=userService.uploadFile(email, fileRealPath, fileHttpPath);
			if(a==1){
